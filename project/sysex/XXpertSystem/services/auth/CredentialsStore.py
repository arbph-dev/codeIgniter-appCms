...
Usage :
    ...
    # Construction directe du bon provider
    auth = store.build_auth("insee")    → ApiKeyAuth
    auth = store.build_auth("inpi")     → BearerAuth
    auth = store.build_auth("zealot")   → BearerAuth
"""
import sqlite3
...
# Permet à build_auth() de savoir comment instancier chaque service
SERVICE_CONFIG = {
    "zealot": {
        "type":       "bearer",
...
# Chemin par défaut : data/credentials.db dans le dossier du projet
...
class CredentialsStore:
    ...
    def get(self, service: str) -> Optional[dict]:
        """
        Retourne les credentials d'un service.
    ...
    # ------------------------------------------------------------------
    # Factory : construit le bon AuthProvider depuis les credentials
    # ------------------------------------------------------------------
    def build_auth(self, service: str) -> Optional[AuthProvider]:
        """
        Construit et retourne le bon AuthProvider pour un service.
        Ne fait PAS le login — appeler auth.login() ou auth.ensure_logged() ensuite.
    ...
    def build_and_login(self, service: str) -> Optional[AuthProvider]:
        ...
        Pour BearerAuth utilise login/password de la table.
        Pour ApiKeyAuth la session est prête directement.
        """
