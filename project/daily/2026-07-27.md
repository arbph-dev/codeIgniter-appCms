...
```sql
-- =============================================================================
-- Seed Iter006.2.3 — Test TabSystem CmsArticleWorkbench
-- =============================================================================
...
--             et is_published = 1.
--
-- Type IDs supposés (à vérifier avec SELECT * FROM component_types) : voir ci dessus
...
INSERT INTO cmssections
    (article_id, slug, title, position, is_published, created_at, updated_at)
SELECT
    a.id,
...
INSERT INTO cmsparts
    (section_id, type_id, title, aside, config, position, is_published, created_at, updated_at)
SELECT
    s.id,
    1,          -- type_id = 1 / raw
    'Présentation', -- title
    '<p>Contenu de la section <strong>Introduction</strong>.</p><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>', -- aside
    '{"content": "<p>Contenu de la section <strong>Introduction</strong>.</p><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>"}',-- config
    1,
    1,
    NOW(), NOW()
FROM cmssections s
...
-- Section 3 : Développement — deux parts pour vérifier l'empilement
INSERT INTO cmsparts
    (section_id, type_id, title,  aside, config, position, is_published, created_at, updated_at)
SELECT
    s.id,
    ...
    '{"content": "<p>Premier paragraphe du <strong>Développement</strong>.</p><p>Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris.</p>"}',
    1,
    1,
    NOW(), NOW()
FROM cmssections s
...
SELECT
    ...
    'Note importante',
    'Ceci est un callout de test — vérifie que le composant callout s''initialise correctement dans ce pane.',
    '{"id": "CO_1", "type": "warning", "title": "Attention", "content": "Ceci est un callout de test de type <strong>warning</strong>.2026-07-07 : Mise en production. Ajouter initCallout dans le bootstrap"}',
    2,
    1,
    NOW(), NOW()
FROM cmssections s
...
SELECT
    ...
    '{"content": "<p>Contenu de la section <strong>Conclusion</strong>.</p><p>Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>"}',
    1,
    1,
    NOW(), NOW()
FROM cmssections s
...
mis a jour les notes
- https://github.com/arbph-dev/codeIgniter-appCms/blob/main/documentation/MIGRATIONS/cmsparts.md
    content était toujours mentionné, il était absent du seeder
    les parts components (raw, calllout) utilise config
- https://github.com/arbph-dev/codeIgniter-appCms/blob/main/project/daily/2026-07-27.md

assets/js/ui/workbench/layouts/CmsArticleWorkbench.js
Fonctionne , bug InitComponent sur le switch des tabs mais c'est anticipé

Pour les composants FIRST /assets/js/components/mermaid.js il disposent d'une fonction reRender(id) 
