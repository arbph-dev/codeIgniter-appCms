...
**spécification fonctionnelle et technique V1.0**, suffisamment générique pour servir de base au développement dans ton CMS/GMAO.
...
1. Référentiel des dimensions physiques
2. Référentiel des unités
3. Gestion des caractéristiques physiques
4. Moteur de calcul des grandeurs
...
1. recherche des caractéristiques disponibles ;
2. conversion éventuelle dans les unités de référence ;
3. recherche des calculs capables de produire la grandeur ;
4. vérification des données disponibles ;
5. exécution de la formule ;
6. création éventuelle d'une nouvelle caractéristique d'origine **CALCUL**.
...
```
Grandeurs d'entrée        │        ▼Formule        │        ▼Grandeur résultat
```
...
```
Débit        │        ▼Puissance thermique        │        ▼Énergie        │        ▼Coût annuel
```
...
# 003_grandeurs_physiques – Note de synthèse
...
Le modèle obtenu constitue un socle métrologique générique, indépendant des domaines métier et facilement extensible.
