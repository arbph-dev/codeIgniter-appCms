...
Usage :
    ...
    # INPI — même interface, login_path différent
    auth = BearerAuth(
...
class BearerAuth(AuthProvider):
    ...
    def login(self, email: str, password: str) -> bool:
        ...
        Essaie d'abord le champ 'email', puis 'username' si 400/422
        (nécessaire pour l'API INPI qui n'accepte que 'username').
        """
        try:
    ...
    def ensure_logged(self, email: str, password: str) -> bool:
        """Vérifie la session via /me, relogin si nécessaire."""
    ...
    def __repr__(self):
        state = (
            f"connecté ({self.user.get('username')})"
            if self.is_logged and self.user
            else "non connecté"
        )
