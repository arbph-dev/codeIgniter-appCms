...
def _rec(organisation_id, siren, local_id, status="saved"):
    """
    NB : local_id est assigné APRÈS construction (rec.local_id = ...),
    jamais via le constructeur — exactement comme le fait le code réel
    dans persist_record()/push_to_zealot(). C'est nécessaire tant que
    WMRecord porte le champ mal nommé "Pythonlocal_id" (bug noté en
    Phase 2, pas corrigé avant la fin de la migration) : le constructeur
    n'accepte pas local_id= en kwarg, mais l'assignation dynamique après
    coup fonctionne quel que soit le nom du champ déclaré.
    """
    rec = WMRecord(
        organisation_id=organisation_id, nom="peu importe", siren=siren,
        type_id=1, type_label="Entreprise", status=status,
    )
