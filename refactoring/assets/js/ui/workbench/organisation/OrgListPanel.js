...
export class OrgListPanel extends PanelBase
{
    constructor()
    {
        super()
    ...
    render()
    {
        ...
        this.inputEl.addEventListener('keydown', e => { if (e.key === 'Enter') this._triggerSearch() })
    ...
    show(items, pager = null)
    {
        ...
        if (!items?.length) { this.tableEl.appendChild(notice('empty')); return }
    ...
    clear()        { if (!this.tableEl) return; clear(this.tableEl); clear(this.pagerEl); this.tableEl.appendChild(notice('empty')) }
    showLoading()  { if (!this.tableEl) return; clear(this.tableEl); clear(this.pagerEl); this.tableEl.appendChild(notice('loading')) }
    showError(msg) { if (!this.tableEl) return; clear(this.tableEl); clear(this.pagerEl); this.tableEl.appendChild(notice('error', msg)) }

    destroy()
    {
        this._onSelectFn = null; this._onSearchFn = null
        this._onNewFn    = null; this._onPageFn   = null
        this.element = null; this.inputEl = null
        this.tableEl = null; this.pagerEl = null
    }
    ...
    _triggerSearch() { this._onSearchFn?.(this.inputEl?.value.trim() ?? '') }
}
